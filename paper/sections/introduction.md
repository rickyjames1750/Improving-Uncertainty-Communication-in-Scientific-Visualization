...
We expect that distributional encodings (violin/box and HOPs) will produce
higher decision accuracy and better confidence calibration than error bars,
because they give viewers more information about the shape and spread of the
underlying data. We also expect that this improvement will not come at a
prohibitive time cost — maybe a few extra seconds per task, but not enough
to be a dealbreaker. The pilot data (n = 4) showed a preliminary pattern
consistent with this: error bars and violin/box had similar accuracy and
timing, while HOPs were slower and slightly less accurate, likely because
the static grid format was harder to parse.
