...
between blocks were used to reduce carry-over effects.  In this
sample, the HOPs grid had the highest aggregate accuracy
(100\% of 48 trials), followed by error bars (97.9\%) and violin/box
plots (91.7\%).  All three encodings were at or near ceiling, so the
100\% figure should be read as ``no errors observed in this small,
high-data-literacy sample on these specific tasks,'' not as evidence
that HOPs are universally perfect.  A participant-level, tie-corrected
Friedman test on accuracy was significant ($\chi^2 = 6.50$, $df = 2$,
$p = 0.039$, Kendall's $W = 0.20$), while response time differences
were not ($\chi^2 = 4.63$, $df = 2$, $p = 0.099$).  Post-hoc pairwise
Wilcoxon tests did not survive Bonferroni correction, so these results
are best read as preliminary signal rather than conclusive evidence.  They are
...
In this sample of 16 participants and 48 trials per encoding, the HOPs
grid had the highest aggregate accuracy (48/48 = 100\%), followed by
error bars (47/48 = 97.9\%) and violin/box (44/48 = 91.7\%).  We use
``100\%'' here in its narrow, descriptive sense: zero errors observed
on these three task types, by these sixteen participants, on this one
clean dataset.  It is not a claim that HOPs produce error-free
judgments in general.  Nine of the sixteen participants were
themselves at 100\% on all three encodings, and every participant was
at ceiling on the estimation and decision task types for error bars
and HOPs.  This compresses what the statistical test can see:
a tie-corrected Friedman test at the participant level was significant
but with a modest effect size ($\chi^2 = 6.50$, $df = 2$, $p = 0.039$,
Kendall's $W = 0.203$), and none of the three post-hoc Wilcoxon
signed-rank tests cleared the Bonferroni-corrected threshold of
$\alpha = 0.017$ (error bars vs.\ violin/box $p = 0.18$, error bars
vs.\ HOPs $p = 1.00$, violin/box vs.\ HOPs $p = 0.10$).  Almost all of
the between-encoding signal came from a single cell: the violin/box
...
task.  Estimation accuracy was $100\%$ in all three encodings, which
makes that task uninformative for separating the conditions.  Given
the high ceiling, the small sample, and the fact that the omnibus
test does not survive a conservative correction, the accuracy result
should be read as preliminary rather than conclusive.

\begin{table}[ht]
\centering
\caption{Decision accuracy by encoding and task type ($n = 16$
  participants, 48 trials per encoding).  Cells at 100\% reflect zero
  errors observed in this sample on these specific tasks, not a
  universal claim about the encoding.}
\label{tab:accuracy}
...
\begin{figure}[ht]
  ...
  \caption{Proportion correct by encoding and task type.  Error bars
    ...
    condition below ceiling, and the bars at 1.0 should be read as
    ``no errors observed in this small sample,'' not as a universal
    accuracy claim.}
  \label{fig:accuracy}
\end{figure}
...
a static HOPs grid produced no observed errors in this sample, while
error bars and violin/box plots produced a small handful.  In the
same sample, error bars were numerically the fastest and
best-calibrated encoding, and violin/box carried a descriptive cost on
the ranking task and on per-trial response time.  Because every
encoding sat at or near ceiling, none of these gaps was large enough
to survive a conservative statistical threshold once we corrected for
multiple comparisons.  We therefore avoid framing the 100\% HOPs
